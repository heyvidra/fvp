...
class MdkVideoPlayer extends mdk.Player {
  ...
  // PATCH(vidra): keep event subscriptions so dispose() can cancel them.
  // Upstream closes streamCtl while the listeners stay subscribed, so events
  // already queued on the source streams fire after close and throw
  ...
  MdkVideoPlayer() : super() {
    _subscriptions.add(onMediaStatus.listen((event) {
      ...
      if (!oldValue.test(mdk.MediaStatus.loaded) &&
        ...
        textureSize.then((size) {
          // PATCH(vidra): async gap — player may be disposed meanwhile.
    ...
    _subscriptions.add(onEvent.listen((ev) {
      _log.fine(
    ...
    _subscriptions.add(onStateChanged.listen((event) {
      _log.fine(
